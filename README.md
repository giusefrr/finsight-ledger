...
Sealed September 14, 2026; last updated September 19, 2026, as the two
LLM engines finished answering the same 405 questions the three
mechanical baselines had already answered.
...
- Each question was submitted to **5 engines**: three mechanical
  ...
  Factors text from the most recent 10-K/10-Q filed on or before the
  anchor date) — for a total of 405 × 5 = 2025 attempts: **1991
  predictions** actually written and **34 declines** declared (an engine
  that explicitly refuses to answer when it doesn't have enough history —
  never a random guess). 1 `question_set` + 1991 `prediction` + 34
...
```
entries.jsonl          # all 2026 ledger entries, one per JSON line, in seq order
anchors_index.jsonl    # human-readable index of the 10 RFC 3161 anchors (see below)
anchors/                # 10 anchors x 3 files each = 30 files, one stem per anchor:
  1_freetsa    2_digicert    3_freetsa    4_digicert    5_freetsa
  6_digicert   7_freetsa     8_digicert   9_freetsa     10_digicert
  # each stem has <stem>.tsr / <stem>.head_hash.txt / <stem>.pem
verify_entry.py         # recomputes the hash of ONE entry (~15 lines, standard library only)
...
Why 10 anchors and not 2: anchors 1 (FreeTSA) and 2 (DigiCert) cover only
`seq=1` — the `question_set`, i.e. the 405 questions and their
thresholds, anchored BEFORE any engine saw them (so the thresholds
cannot have been picked with hindsight). From there the ledger grew in
four more anchored batches, each pair covering the ledger exactly as it
stood right after a batch of predictions/declines was written — never
left unanchored, per FinSight's own discipline: anchors 3–4 at `seq=1216`
(the three baselines' first pass: 1197 predictions + 18 declines);
anchors 5–6 at `seq=1984`; anchors 7–8 at `seq=2009`; and anchors 9–10 at
`seq=2026` — the current, full ledger: 1991 predictions and 34 declines
across all five engines. Every pair uses the same two services (FreeTSA,
DigiCert), so this ledger's credibility never rests on either alone. The
four earlier anchors (1–2, 3–4) still verify successfully too — they
simply cover smaller, earlier snapshots of the same append-only chain,
proving the baseline-only phase existed before the two LLM engines were
added.
...
Exact command, one per anchor — replace `<pair>` with one of the ten
prefixes (`1_freetsa`, `2_digicert`, `3_freetsa`, `4_digicert`,
`5_freetsa`, `6_digicert`, `7_freetsa`, `8_digicert`, `9_freetsa`,
`10_digicert`):
...
CURRENT, FULL ledger — all five engines, DigiCert service):

```bash
openssl ts -verify -in anchors/10_digicert.tsr \
  -data anchors/10_digicert.head_hash.txt \
  -CAfile anchors/10_digicert.pem
```

Expected output: `Verification: OK`. All 10 anchors in this directory
were verified this way, one by one, from an isolated copy of just this
directory's contents, right before this README was published — none
returns anything other than `Verification: OK`.

To verify all ten in one shot (no dependency beyond `openssl` — no
`jq`):
...
Italian (the ledger's original working language, before it was
translated to English — the chain proves this entry existed unchanged
since it was written, not what language it was written in), so you can
compare it character for character against the real line in
`entries.jsonl`. Its `reason` field, `"storia insufficiente: servono
almeno 8 osservazioni storiche, disponibili 7"`, translates as:
"insufficient history: at least 8 historical observations required, 7
available".
...
To re-verify the ENTIRE chain, all 2026 entries, in one shot —
...
Expected output: `Chain intact: 2026 entries verified, from the first
(prev_hash=64 zeros) to seq=2026.` followed by the final `head_hash`
(`13b653cd8647b3b627bda4772c8bff44f8b5f99e51072b31bf7a6642da1e1e8b`). To
...
field of `anchors_index.jsonl`) — here, the final anchor pair, which
covers the entire current ledger:

```bash
python3 verify_chain.py entries.jsonl "$(cat anchors/10_digicert.head_hash.txt)"
```

Expected output: in addition to the line above, `MATCH with expected
head_hash (13b653cd8647b3b627bda4772c8bff44f8b5f99e51072b31bf7a6642da1e1e8b)`.
Run exactly as written: the chain recomputed from `entries.jsonl` alone
lands exactly on the head_hash that anchor 10 had DigiCert sign — the two
verifications together (this one + `openssl ts -verify` above) are what
proves that all 2026 entries existed, unchanged, no later than the
moment signed by that anchor.
...
- **`prediction`**: `prediction_id`, `question_set_id` (which question
  ...
  date by which it resolves), `status` (`PENDING` for all of them here —
  none resolved yet), and the `resolved_at`/`resolved_value`/
...
**It proves**: that these 405 questions and their thresholds existed,
textually identical to how you read them here, **no later than September
14, 2026, 14:12 UTC** (anchors 1–2, before any engine saw them, so the
thresholds cannot have been picked with hindsight); that the first batch
of 1197 baseline predictions (the three mechanical engines) existed no
later than **September 14, 2026, 21:07 UTC** (anchors 3–4); and that the
full current ledger — **1991 predictions and 34 declines across all five
engines** — existed, unchanged, **no later than September 19, 2026,
04:24 UTC** (anchors 9–10, the latest anchor pair; anchors 5–6 and 7–8
cover two intermediate snapshots along the way, at `seq=1984` and
`seq=2009` respectively). This is not a claim we're making: it is what
`openssl ts -verify` — an independent tool, one that doesn't require
trusting this code — confirms above, combined with the hash chain
recomputed by hand from this same file. No line could have been altered
after the relevant date without breaking the chain at a point detectable
by anyone, with the same two commands.
...
- **Anything about what happens after `covered_seq`**: an anchor covers
  ...
  them) — exactly the pattern behind anchors 3 through 10 above.
- **The resolutions**: no question here is resolved yet (all `PENDING`);
  the accuracy of the five engines can only be judged once the actual
